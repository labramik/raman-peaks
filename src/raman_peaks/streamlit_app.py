...
def _peaks_to_csv(peaks) -> bytes:
    buf = io.StringIO()
    writer = csv.DictWriter(buf, fieldnames=["center", "amplitude", "fwhm", "model", "aic", "rss"])
    writer.writeheader()
...
def main():
    ...
    st.write("Upload a two-column TXT spectrum (shift, intensity). The app will detect and fit peaks using only the top fraction of each peak.")

    with st.sidebar:
        st.header("Parameters")
        top_fraction = st.slider("Top fraction of peak height", min_value=0.05, max_value=0.6, value=0.2, step=0.05)
        k_noise = st.slider("Sensitivity (k x noise)", min_value=1.0, max_value=10.0, value=5.0, step=0.5)
        smoothing_window = st.slider("Smoothing window (points)", min_value=5, max_value=51, value=11, step=2)
        baseline_on = st.checkbox("Apply baseline (rolling median)", value=True)
        baseline_window = st.slider("Baseline window (points)", min_value=11, max_value=201, value=51, step=2)
        peak_window = st.slider("Peak fit window (cm^-1)", min_value=5.0, max_value=80.0, value=30.0, step=1.0)
        shoulders_on = st.checkbox("Detect shoulders (relaxed threshold)", value=True)
        shoulder_height_ratio = st.slider("Shoulder height ratio vs main threshold", min_value=0.2, max_value=0.8, value=0.5, step=0.05)
    ...
    if upload:
        ...
        st.info(
            f"Detected {len(result.peaks)} peaks | noise sigma ~ {result.noise_sigma:.3g} | top fraction {top_fraction:.2f}"
        )
        ...
        with col_plot:
            ...
            st.download_button("Download plot (PNG)", data=png_bytes, file_name="raman_peaks.png", mime="image/png")
        with col_table:
            if result.peaks:
