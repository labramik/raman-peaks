...
def voigt_profile(
    ...
    return amplitude * np.real(special.wofz(z)) / (sigma * np.sqrt(2 * np.pi))  # pylint: disable=no-member
...
@dataclass
class PeakFit:  # pylint: disable=too-many-instance-attributes
    """Fitted peak parameters and evaluation helper."""
    center: float
    amplitude: float
    fwhm: float
    model: str
    ...
    def evaluate(self, x: np.ndarray) -> np.ndarray:
        """Evaluate the fitted peak at coordinates x."""
...
def _fwhm_from_params(model: str, params: Dict[str, float]) -> float:
    ...
    term = 2.0 * np.sqrt(2 * np.log(2)) * sigma
    return 0.5346 * 2 * gamma + np.sqrt(0.2166 * (2 * gamma) ** 2 + term**2)
...
def select_top_fraction(
    ...
    """Keep only the top fraction of a peak for fitting."""
...
def _fit_model(
    ...
    """Fit a specific model to peak data."""
    ...
    try:
        if model == "lorentzian":
            ...
            bounds = (
                [center_guess - width_guess * 2, 1e-6, 0],
                [center_guess + width_guess * 2, np.inf, np.inf],
            )
            ...
            return (
                PeakFit(params["x0"], params["amplitude"], fwhm, model, aic, rss, len(x), params),
                y_fit,
            )
        p0 = [
            center_guess,
            width_guess / 3.0,
            max(width_guess, 1e-3),
            max(height_guess, 1e-6),
        ]
        bounds = (
